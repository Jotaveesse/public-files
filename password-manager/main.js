...
window.onload = function () {
    ...
    document.getElementById("display-button").onclick = async function () {
        if (!busy) {
            ...
            var decrData = await decryptDefault(
                inputArea.value,
                passwordArea.value,
            );
            var jsonData;
...
function createPersonElems(data, parent, isNew = false) {
    Object.keys(data).forEach((person) => {
        ...
        const newAccountButtonWrapper = newAccountButton.parentElement;
        ...
        newAccountButton.onclick = function () {
            createAccountElems(emptyData.Person, personAccounts);
        };
...
function createAccountElems(person, parent) {
    ...
    Object.keys(person).forEach((account) => {
        ...
        newLoginButton.onclick = function () {
            createLoginElems(emptyData.Person.Account, accountData);
        };
    ...
    var sorted = accountElems.sort((a, b) =>
...
function createLoginElems(account, parent) {
    account.forEach((login) => {
        ...
        parent.querySelector(".new-account-button").before(loginElem);
    });
}
...
function extractJson() {
    ...
    Array.from(jsonArea.children).forEach((personElem) => {
        ...
        Array.from(personAccounts.children).forEach((accountElem) => {
            const accountData = accountElem.querySelector(".account-data");
            const accountTitle =
                accountElem.querySelector(".account-title").innerHTML;
            extractedJson[personTitle][accountTitle] = [];

            //extracts each login
            Array.from(accountData.querySelectorAll(".data-row")).forEach(
                (loginElem) => {
                    const loginField = loginElem.querySelector(".login-field");
                    const passwordField =
                        loginElem.querySelector(".password-field");

                    const loginInput =
                        loginField.getElementsByTagName("input")[0];
                    const passwordInput =
                        passwordField.getElementsByTagName("input")[0];
...
function updateProgressBar(progress) {
    if (progress == 1) progressBar.style.display = "none";
    else {
